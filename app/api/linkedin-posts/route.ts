...
export async function GET() {
  try {
    ...
    const collection = db.collection('linkedin-posts')
    ...
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to fetch LinkedIn posts from database' 
      },
      { status: 500 }
    )
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`✅ LinkedIn post saved to database: ${result.insertedId}`)
    ...
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to save LinkedIn post to database' 
      },
      { status: 500 }
    )
  }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`✅ LinkedIn post deleted: ${postId}`)
    ...
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to delete LinkedIn post from database' 
      },
      { status: 500 }
    )
  }
