...
//   console.error('MONGODB_URI="your-uri" MONGODB_DATABASE="your-db" node scripts/updateInstagramData.js');
//   process.exit(1);
// }
...
async function updateInstagramData() {
  ...
  try {
    ...
    const updatedPostsStats = await citizensCollection.aggregate([
      {
        $group: {
          _id: '$socialMedia.instagram.postsCount',
          count: { $sum: 1 }
    ...
    const updatedAvgLikesStats = await citizensCollection.aggregate([
      {
        $group: {
          _id: '$socialMedia.instagram.avgLikes',
          count: { $sum: 1 }
    ...
    const updatedAvgCommentsStats = await citizensCollection.aggregate([
      {
        $group: {
          _id: '$socialMedia.instagram.avgComments',
          count: { $sum: 1 }
    ...
    updatedPostsStats.forEach(stat => {
      console.log(`   📸 ${stat._id} posts: ${stat.count} citizens`);
    });
    
    console.log('\n📊 Updated Instagram AvgLikes Distribution:');
    updatedAvgLikesStats.forEach(stat => {
      console.log(`   ❤️  ${stat._id} avg likes: ${stat.count} citizens`);
    });
    
    console.log('\n📊 Updated Instagram AvgComments Distribution:');
    updatedAvgCommentsStats.forEach(stat => {
      console.log(`   💬 ${stat._id} avg comments: ${stat.count} citizens`);
    });
    ...
    const zeroPostsCount = updatedPostsStats.find(stat => stat._id === 0)?.count || 0;
    const zeroAvgLikesCount = updatedAvgLikesStats.find(stat => stat._id === 0)?.count || 0;
    const zeroAvgCommentsCount = updatedAvgCommentsStats.find(stat => stat._id === 0)?.count || 0;
